...
public class BarflakeGenerator implements IdGeneratorSync {
	...
	/** */
	public BarflakeGenerator(CurrentTimeMillisProvider timer, int datacenterId, int workerId) {
	...
	private void update() throws InvalidSystemClock {
		...
		if (currentTime == this.timestamp) {
			sequence++;
			if (sequence > SEQ_MAX) { throw new IllegalStateException("Too many ids created for this millisecond"); }
		}
			...
			if (currentTime > this.timestamp) {
				...
				if (currentTime < this.timestamp) { throw new InvalidSystemClock(
				    "current time is before last used timestamp!"); }
